...
Considerando las ventajas y desventajas de las distintas formas de codificación se sugiere utilizar valores perdidos con valores altos en negativo de modo tal que sean estándar para todas las variables y no sean confundible con los valores posibles de otras variables. Se recomienda utilizar la recodificación presente en la Tabla N° \@ref(tab:tabna), usando valores numéricos o caracteres según corresponda.
...
| No disponible     | -994            |
Table: `r as.character(paste("(\\#tab:tabna)", "Recomendacion de recodificación para valores perdidos."))`. <!-- Label+titulo de tabla en syntax Markdown pura -->
